        ...
        for STA in [os.path.split(_e)[-1] for _e in RAW_STA_DIRS]:
            ...
            for _R, _C in [(RAWZ,'Z'),(RAWN,'N'),(RAWE,'E')]:
                ...
                for _f in flist:
                    ...
                    savename = f"{NET}.{STA}.{LOC}.{CBI}{_C}_{tr.stats.starttime.strftime('%Y%m%d_%H%M%S')}.{format.lower()}"
                    print(f'Writing: {DATA/savename}')
                    tr.write(str(DATA/savename), format=format.upper())            
