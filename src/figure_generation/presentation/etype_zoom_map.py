...
event_max_radius_km = 10.
map_radius_km = 8.

bounds = rad2llur(rad = event_max_radius_km*1e3)
...
axm, map_attrib = mount_baker_basemap(fig=fig, sps=gs[:-2, :3], radius_km=map_radius_km,
                                      open_street_map=False, zoom=12)
...
gl.xlocator = mticker.FixedLocator(np.arange(-123, 121, 0.05))
...
for _e, _c in [('eq','ko'), ('lf','rs'), ('su','bs'), ('px','co'), ('uk', 'mo')]:
    ...
    sm = _c[1]
    # Subset event types
    _df = df_eb[(df_eb.etype == _e) & (df_eb.CAT0) & (df_eb.offset_km <= event_max_radius_km)]
    ...
    axe.plot(_df.horizontal_uncertainty*1e-3, _df.vertical_uncertainty*1e-3, _c,
             alpha=0.5, label=f'{_e.upper()}: {len(_df)}')
...
axe.text(xlims[0]*1.3, 33, '31.2 km', color='m')
axe.set_xlim(xlims)
axe.set_ylim(ylims)
...
if issave:
    ...
    SAVENAME = SAVEPATH / f'etype_zoom_map_gray_{DPI}dpi.{FMT}'
