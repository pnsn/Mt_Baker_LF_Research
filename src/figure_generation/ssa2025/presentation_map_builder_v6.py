...
SDIR = ROOT/'results'/'figures'/'SSA2025'/'v6'
...
### GROUPING RESULTS
zo = 12
### Events Colored By Grouping Including Unanalyzed and Ungrouped
for _grp in df_cat.tidy_group.unique():
    _df = df_cat[df_cat.tidy_group == _grp]
    for _et in _df.etype.unique():
        if _grp == -2:
            color = unanalyzed_color
        elif _grp == -1:
            color = ungrouped_color
        ...
        zo += 1
if issave:
    fig.savefig(str(SDIR/f'events_colored_by_group_{DPI}dpi.{FMT}'),**sfkw)
...
for _pet in ['su','lf','px','eq']:
    ...
    for _et, _ret in _df[['etype','petype']].value_counts().index:
        if _et == _ret:
            ...
            _zo += 1
    if issave:
        fig.savefig(str(SDIR/f'proposed_{_pet}_reassignment_map_{DPI}dpi.{FMT}'),
                    **sfkw)
    hdls = clear_handles(hdls)


### SUMMARY FIGURE WITH PROPOSED CHANGES APPLIED
# for _pet in ['eq','su','lf','px']:
#     handles = plotfun(
#         axm, axt, df_cat[(df_cat.petype==_pet) & (df_cat) ]
#     )
...
if makemovie:
    ...
    while t1 <= T1:
        ...
        ihdl = axm.scatter(ilons, ilats, marker='v', edgecolors='k', c='w', s=36,
        ...
        for _pet in ['eq','su','lf','px']:
            ...
            # Plot grouped
            __df = _df[(_df.petype==_pet)&(_df.tidy_group>0)]
            if len(__df) > 0:
                __alphas = [1 - (t1 - row.prefor_time)/DTWIN for _, row in __df.iterrows()]
                hdl = plotmap(
                    axm,
        ...
        # Save
        if issave:
            savename = SDIR.parent/'reclassified_movie'/f'frame_{frame_no:05d}_{MDPI:d}dpi.{FMT}'
            fig.savefig(str(savename), **sfkw)
        # Advance timer
        t0 += DTSLIDE
        t1 += DTSLIDE
